...
import { SIM_LABELS, STOCK_ACTION_LABELS } from '../constants';
...
export const Inventory: React.FC = () => {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const available = action === 'recover' ? damaged[type] : stock[type];
    const isNegativeAction = ['return_company', 'to_damaged', 'recover', 'flush'].includes(action);
    if (isNegativeAction && available < quantity && action !== 'recover') {
      setError('لا يوجد مخزون كافٍ لتنفيذ العملية');
      setSuccess('');
      return;
    }
    if (action === 'recover' && damaged[type] < quantity) {
      setError('لا يوجد تالف كافٍ للاسترجاع');
      setSuccess('');
      return;
    }
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          {stockLogs.slice().reverse().map((log) => (
            <div key={log.id} style={{ padding: 10, borderRadius: 10, border: '1px solid #e2e8f0', background: '#f8fafc' }}>
              ...
              <div style={{ fontSize: 14 }}>الإجراء: {STOCK_ACTION_LABELS[log.action]} — كمية: {log.quantity}</div>
            </div>
          ))}
