...
export const Reports: React.FC = () => {
  const { transactions, removeTransaction, addTransaction, stock, getPriceForQuantity } = useData();
  ...
  const totals = useMemo(() => {
    ...
    const todayData = transactions.filter((t) => t.date.slice(0, 10) === today);
    const todayRevenue = todayData.reduce((sum, t) => sum + t.amount, 0);
    const todayQty = todayData.reduce((sum, t) => sum + t.quantity, 0);
    const overallRevenue = transactions.reduce((sum, t) => sum + t.amount, 0);
    const overallQty = transactions.reduce((sum, t) => sum + t.quantity, 0);
    return { todayRevenue, todayQty, overallRevenue, overallQty };
  }, [transactions]);

  const isSimType = type === 'jawwy' || type === 'sawa' || type === 'multi';
  const suggestedUnitPrice = useMemo(() => {
    if (!isSimType) return 0;
    return getPriceForQuantity(type as SimType, quantity || 1);
  }, [getPriceForQuantity, isSimType, quantity, type]);

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    const result = addTransaction(type, amount || suggestedUnitPrice * quantity, quantity, { autoPrice: true });
    if (!result.ok) {
      setError(result.error || 'تعذر حفظ العملية');
      setSuccess('');
      return;
    }
    setError('');
    setSuccess(`تم الحفظ. المبلغ المسجل: ${result.chargedAmount} ريال`);
    setAmount(0);
    setQuantity(1);
  };
      ...
      <section style={{ background: 'white', padding: 16, borderRadius: 12, border: '1px solid #e2e8f0' }}>
        ...
        {isSimType && (
          <div style={{ marginTop: 8, color: '#475569', fontSize: 13 }}>
            سعر مقترح: {suggestedUnitPrice} ريال للقطعة — الإجمالي المتوقع: {suggestedUnitPrice * quantity} ريال | الرصيد المتاح: {stock[type as SimType]} قطعة
          </div>
        )}
        {error && <div style={{ marginTop: 8, color: '#ef4444' }}>{error}</div>}
        {success && <div style={{ marginTop: 8, color: '#10b981' }}>{success}</div>}
      </section>

      <section style={{ background: 'white', padding: 16, borderRadius: 12, border: '1px solid #e2e8f0' }}>
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 8 }}>نظرة سريعة</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
          <div style={{ padding: 12, borderRadius: 10, background: '#f8fafc', border: '1px solid #e2e8f0' }}>
            <div style={{ color: '#475569', fontSize: 12 }}>اليوم</div>
            <div style={{ fontWeight: 700, fontSize: 20 }}>{totals.todayRevenue} ريال</div>
            <div style={{ fontSize: 12, color: '#475569' }}>عدد العمليات: {totals.todayQty}</div>
          </div>
          <div style={{ padding: 12, borderRadius: 10, background: '#f8fafc', border: '1px solid #e2e8f0' }}>
            <div style={{ color: '#475569', fontSize: 12 }}>الإجمالي</div>
            <div style={{ fontWeight: 700, fontSize: 20 }}>{totals.overallRevenue} ريال</div>
            <div style={{ fontSize: 12, color: '#475569' }}>عدد العمليات: {totals.overallQty}</div>
          </div>
        </div>
      </section>
