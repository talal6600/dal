...
export const SettingsPage: React.FC = () => {
  ...
  const [preferredFuelType, setPreferredFuelType] = useState(settings.preferredFuelType);
  const [priceConfig, setPriceConfig] = useState(() => ({
    jawwy: [...settings.priceConfig.jawwy],
    sawa: [...settings.priceConfig.sawa],
    multi: [...settings.priceConfig.multi],
  }));
  ...
  const saveSettings = (e: React.FormEvent) => {
    e.preventDefault();
    updateSettings({ name, weeklyTarget, preferredFuelType, priceConfig });
    setMessage('تم حفظ الإعدادات');
  };

  const updatePrice = (key: keyof typeof priceConfig, tierIndex: number, value: number) => {
    setPriceConfig((prev) => ({
      ...prev,
      [key]: prev[key].map((item, idx) => (idx === tierIndex ? value : item)) as typeof prev[typeof key],
    }));
  };
    ...
    <div style={{ display: 'grid', gap: 16 }}>
      <section style={{ background: 'white', padding: 16, borderRadius: 12, border: '1px solid #e2e8f0' }}>
        ...
        <form onSubmit={saveSettings} style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
          ...
          <label style={{ display: 'grid', gap: 6 }}>
            <span>نوع الوقود المفضل</span>
            <select value={preferredFuelType} onChange={(e) => setPreferredFuelType(e.target.value as typeof preferredFuelType)} style={{ padding: 10, borderRadius: 10, border: '1px solid #cbd5e1' }}>
              {Object.keys(FUEL_LABELS).map((key) => (
                <option key={key} value={key}>{FUEL_LABELS[key as keyof typeof FUEL_LABELS]}</option>
              ))}
            </select>
          </label>
      ...
      <section style={{ background: 'white', padding: 16, borderRadius: 12, border: '1px solid #e2e8f0' }}>
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 10 }}>تسعيرة الشرائح (آلي)</div>
        <p style={{ margin: 0, color: '#475569', fontSize: 13 }}>يتم اقتراح السعر تلقائياً في صفحة التقارير حسب الكمية (1، ثم 5، ثم 10+).</p>
        <div style={{ display: 'grid', gap: 12, marginTop: 12 }}>
          {Object.entries(priceConfig).map(([key, tiers]) => (
            <div key={key} style={{ display: 'grid', gap: 6 }}>
              <div style={{ fontWeight: 600 }}>{key.toUpperCase()}</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8 }}>
                {tiers.map((value, idx) => (
                  <label key={idx} style={{ display: 'grid', gap: 4 }}>
                    <span style={{ fontSize: 12, color: '#475569' }}>{idx === 0 ? '1-4 قطع' : idx === 1 ? '5-9 قطع' : '10+ قطع'}</span>
                    <input type="number" min={0} value={value} onChange={(e) => updatePrice(key as keyof typeof priceConfig, idx, Number(e.target.value))} style={{ padding: 10, borderRadius: 10, border: '1px solid #cbd5e1' }} />
                  </label>
                ))}
              </div>
            </div>
